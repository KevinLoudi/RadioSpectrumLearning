...
for j=1:length(confidence_arr)
    z_alph=z_alph_arr(j);
for ii=1:exp_time
    [s,rcs,rr(1,ii)]=Generate_simulation_dataset_v2(10,3,len,20,0.5,4); %import noise parameters with total length
...
%% 
 ...
 hold on; plot(confidence_arr,p_md,'LineWidth', 1.0); xlabel('置信度', 'FontSize',12); ylabel('概率指标', 'FontSize',12);
 legend('误警概率','漏检概率', 'FontSize',12);
 axis([0.75 1 0 1]);
...
%  n=8; lamda=5; p=0.8; n_mu=10; n_theata=3; %heave load, random occupay
 n=8; lamda=5; p=0.8; n_mu=35; n_theata=3;
