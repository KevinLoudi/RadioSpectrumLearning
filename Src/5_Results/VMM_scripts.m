...
figure(1);
subplot(3,1,1); plot(err_arr(1:4:end)); xlabel('训练序列长度'); ylabel('预测误差');
%print('Figs/vmm_train','-dpng','-r500');
subplot(3,1,2); plot(train_time_arr(1:4:end)); xlabel('训练序列长度'); ylabel('训练用时/sec');
%print('Figs/vmm_train_time','-dpng','-r500');
subplot(3,1,3); 
plot(predict_time_arr(1:4:end)); xlabel('训练序列长度'); ylabel('预测用时/sec');
print('Figs/vmm_train','-dpng','-r500');

%% Prediction ability influenced by environment
lamda_arr=[10 20 40 60 80];
p_arr=[0.05 0.1 0.2 0.4 0.6 0.8];
len=1e5;
...
figure(1);
for j=1:length(lamda_arr)
    plot(p_arr,err_arr(:,j),'LineWidth',1.0); hold on;
end
xlabel('用户平均占用时长p'); ylabel('预测误差');
legend('\lambda = 10','\lambda = 20','\lambda = 40','\lambda = 60 ','\lambda = 80')
print('Figs/vmm_environment','-dpng','-r500');
